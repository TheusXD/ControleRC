...
class FirebaseService:
    ...
    def get_docs(self, collection: str, filters: Optional[List[Tuple]] = None) -> pd.DataFrame:
        try:
            ...
            if filters:
                for f in filters:
                    query = query.where(filter=firestore.FieldFilter(f[0].strip(), f[1], f[2]))
        ...
        try:
            ...
            for key, value in new_data.items():
                ...
                if (old_value or "") != (value or ""):
                    history_log.append(
                        f"'{key.replace('_', ' ').capitalize()}' alterado de '{old_value}' para '{value}' por {username} em {now_str}")
            ...
            for collection, records in backup_data.items():
                if collection in collections:
                    for record in records:
                        ...
                        if collection == 'users' and 'password' in processed_record and isinstance(
                                processed_record.get('password'), str):
                            processed_record['password'] = base64.b64decode(processed_record['password'])
                        if collection == 'users' and 'salt' in processed_record and isinstance(
                                processed_record.get('salt'), str):
...
def to_excel(df: pd.DataFrame, title: str = "Relatório") -> bytes:
    output = io.BytesIO()
    df_copy = df.copy()
    for col in df_copy.select_dtypes(include=['datetimetz']).columns: df_copy[col] = df_copy[col].dt.tz_localize(None)
...
class ViewManager:
    ...
    def _init_session_state(self):
        defaults = {'logged_in': False, 'username': "", 'role': "", 'page': "Login", 'user_data': {},
                    'confirm_delete': {}, 'edit_id': None, 'edit_user_id': None, 'confirm_delete_user': {},
                    'reset_password_for_user': {}, 'focus_item': None, 'view_history_id': None,
                    'generate_pedido_from_rc': None, 'confirm_restore': None, 'show_notifications': False,
                    'notifications_list': [], 'editing_comment': None, 'confirm_delete_comment': None}
    ...
    def _format_comment_text(self, text: str, all_users_df: pd.DataFrame) -> str:
        ...
        if not user_mentions: return text
        valid_usernames = all_users_df['username'].tolist() if not all_users_df.empty else []
        for username in user_mentions:
            if username in valid_usernames: text = text.replace(f"@{username}", f"<strong>@{username}</strong>")
        return text
    ...
    def render_data_row(self, row: pd.Series, collection: str, **kwargs):
        ...
        with st.container(border=True):
            if collection == 'demandas':
                assigned_to, assigned_str = row.get(
                    'assigned_to'), f" | **Atribuído a:** `{row.get('assigned_to')}`" if row.get('assigned_to') else ""
    ...
    def _render_comments_section(self, row, collection, **kwargs):
        ...
        if not comentarios:
            ...
            for comment in sorted(comentarios, key=lambda c: c['timestamp']):
                comment_id, is_author, is_admin = comment.get('id'), comment[
                                                                         'username'] == st.session_state.username, st.session_state.role == 'admin'
                    ...
                    with st.form(key=f"edit_form_{comment_id}"):
                        ...
                        if c1.form_submit_button("Salvar"):
                            for c_item in comentarios:
                                if c_item.get('id') == comment_id: c_item['text'], c_item[
                                    'edited_at'] = edited_text, datetime.now(); break
                            if self.db.update_doc(collection, row['id'], {"comentarios": comentarios},
                                ...
                                st.rerun()
                        if c2.form_submit_button("Cancelar"): st.session_state.editing_comment = None; st.rerun()
                else:
                    col1, col2 = st.columns([0.9, 0.1])
                    with col1:
                        with st.chat_message(name=comment['username']):
                            st.write(f"**{comment['username']}** em {comment['timestamp'].strftime('%d/%m/%Y %H:%M')}")
                            st.markdown(self._format_comment_text(comment['text'], all_users), unsafe_allow_html=True)
                            if 'edited_at' in comment: st.caption(
                                f"(editado em {pd.to_datetime(comment['edited_at']).strftime('%d/%m/%Y %H:%M')})")
                    if comment_id:
                        with col2:
                            if is_author:
                                if st.button("✏️", key=f"edit_btn_{comment_id}",
                                             help="Editar"): st.session_state.editing_comment = comment_id; st.rerun()
                            if is_author or is_admin:
                                if st.button("🗑️", key=f"delete_btn_{comment_id}",
                                             help="Excluir"): st.session_state.confirm_delete_comment = {
                                    'collection': collection, 'doc_id': row['id'], 'comment_id': comment_id}; st.rerun()
        ...
        if st.session_state.confirm_delete_comment and st.session_state.confirm_delete_comment['doc_id'] == row['id']:
            ...
            if c2.button("Cancelar",
                         key=f"cancel_del_comment_{st.session_state.confirm_delete_comment['comment_id']}"): st.session_state.confirm_delete_comment = None; st.rerun()
                ...
                for mentioned_user in set(re.findall(r'@(\w+)', new_comment_text)):
                    if mentioned_user in (all_users_df[
                        ...
                        self._create_mention_notification(mentioned_user=mentioned_user,
                                                          ...
                                                          doc_id=row['id'])
                if self.db.update_doc(collection, row['id'], {"comentarios": comentarios + [comment_data]},
                                      st.session_state.username):
    ...
    def render_generate_pedido_modal(self):
        ...
        st.write(
            f"Gerando pedido para a RC: **{rc_data.get('numero_rc', 'S/N')}** | Valor: **{format_brazilian_currency(rc_data.get('valor', 0))}**")
        with st.form("generate_pedido_form"):
            ...
            email_notificacao, anexo_email_file = st.text_area("E-mails para Notificação (opcional)"), st.file_uploader(
                "Anexo para E-mail (opcional)")
                ...
                with st.spinner("Gerando pedido..."):
                    ...
                    try:
                        pedido = Pedido(requisicao_id=rc_data['id'], solicitante=rc_data['solicitante'],
                                        ...
                                        email_notificacao=email_notificacao or None, anexo_email=anexo_email_data)
    ...
    def render_logs_tab(self):
        ...
        if selected_action != "Todas": filtered_df = filtered_df[filtered_df['action'] == selected_action]
        st.dataframe(filtered_df[['timestamp', 'username', 'action', 'details']], use_container_width=True,
                     column_config={
